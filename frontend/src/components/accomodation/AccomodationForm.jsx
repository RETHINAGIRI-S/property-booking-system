    ...
    onSubmit: async ({ value }) => {
      try {
        const newAccomodation = {
          ...
          roomType: value.roomType || "Anytype",
          extraInfo: value.extraInfo || "Check-in on time",
          images: value.images || [],
          address: value.address || {},
          amenities: value.amenities || [],
          checkInTime: value.checkIn || "11:00",
          checkOutTime: value.checkOut || "13:00",
          maximumGuest: Number(value.maximumGuest) || 1,
          price: Number(value.price) || 500,
        };

        const response = await axiosInstance.post("/v1/rent/user/newAccommodation", newAccomodation);
        toast.success("New Property Created Successfully");
        navigate("/accomodation");
      } catch (error) {
        const msg = error.response?.data?.message || error.message || "Failed to create accommodation";
        toast.error(msg);
        console.error(error);
      }
    },
